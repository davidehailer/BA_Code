...
NobBS_d <- function(data, now, units, onset_date, report_date, moving_window=NULL, max_D=NULL, cutoff_D=NULL, proportion_reported=1, quiet=TRUE,
                  specs=list(
                    dist=c("Poisson","NB"),
                    alpha1.mean.prior=0,
  ...
  t.extract <- (now.T-(now.T-1)):(now.T) # nowcast all weeks up through the present # was soll das denn?
  print(t.extract)
